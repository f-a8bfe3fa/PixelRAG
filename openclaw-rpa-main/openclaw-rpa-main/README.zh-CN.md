...
* **📄 原生 Office 自动化：** 内置 `excel_write` 和 `word_write`。**无需安装 Microsoft Office。** 在 macOS、Linux、Windows 上均可无 Office 运行，亦适合 Linux/Docker 部署。
* **🔗 无缝集成：** 作为 OpenClaw 生态系统中的强大技能设计，同时生成标准的 Python/Playwright 代码。

### 运行环境（平台）

**支持 macOS、Linux 与 Windows。** 录制与回放依赖 Python 3、Playwright 及纯 Python 库（`httpx`、`openpyxl`、`python-docx`），上述栈在 Windows 上均有官方支持。在 Windows 上请用 PowerShell 或「命令提示符」执行 `python` / `py -3`（等价于 Unix 上的 `python3`）。示例中的 `~/Desktop` 等路径会通过 `Path.expanduser()` / `os.path.expanduser()` 解析（在 Windows 下通常对应用户目录下的「桌面」等）。自动化测试以 macOS / Linux 为主；若在 Windows 上遇到独有环境问题，欢迎在 GitHub 提 issue。
...
| **部署环境** | **Windows & MS Office 依赖** | **灵活** (但运行成本高) | **macOS / Linux / Windows — Python + Playwright；无 Office；可 Docker** |
...
未列在表里的系统库：一般只需本机已有 Python 3.8+；**macOS / Linux / Windows** 常见环境即可。若只做纯网页录制且确定不用 Excel/Word，技术上可只装 `playwright`+`httpx`，但**推荐**仍使用完整 `requirements.txt`，避免 `record-step excel_write` 时报缺包。
